...
def run(
    ...
    if not training:
        ...
        task = task if task in ('train', 'val', 'test') else 'report'  # path to train/val/test images
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/StemEnd2.yaml', help='dataset.yaml path')
    ...
    parser.add_argument('--task', default='report', help='train, val, test, speed or study or report')
...
def main(opt):
    ...
    if opt.task in ('train', 'val', 'test','report'):  # run normally
